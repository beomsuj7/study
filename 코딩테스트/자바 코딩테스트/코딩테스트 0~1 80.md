...
// StringBuilder -> char -> != -> .append => .toString
class Solution {
...
//.replaceAll -> ("a","b"); a를 b로 대체한다.
        for(char ch : str.toCharArray()) {
            answer += Character.getNumericValue(ch);
        }
// str.toCharArray(): String 객체인 str을 char 배열로 변환합니다. 이를 통해 str의 각 문자를 하나씩 가져올 수 있습니다.

// for(char ch : str.toCharArray()): char 배열로 변환된 str의 각 문자(ch)를 하나씩 순회합니다. ch 변수는 반복문의 각 반복에서 순차적으로 char 배열의 요소로 초기화됩니다.

// Character.getNumericValue(ch): ch 변수에 저장된 문자가 숫자인 경우 해당 숫자 값을 반환합니다. 예를 들어, ch가 '1'인 경우 1을 반환합니다.

// answer += Character.getNumericValue(ch): ch 변수에 저장된 문자가 숫자인 경우 해당 숫자 값을 answer 변수에 더합니다. += 연산자는 기존 answer 값에 Character.getNumericValue(ch)의 결과를 더하여 answer를 갱신합니다.
        return answer;
    }
    ...
    public int solution(int[] absolutes, boolean[] signs) {
        ...
        for (int i = 0; i < absolutes.length; i++) {
            if (signs[i]) { // signs[i]가 참이면 양수이므로 그대로 더한다
                sum += absolutes[i];
            } else { // signs[i]가 거짓이면 음수이므로 -를 더하면 더한것이된다.
                sum -= absolutes[i];
            }
        }
        return sum;
...
class Solution {
    public String solution(String bin1, String bin2) {
        ...
        return sb.reverse().toString();
        // 이진수를 의미하는 문자열 reverse
    }
}


길이가 같은 두 1차원 정수 배열 a, b가 매개변수로 주어집니다. a와 b의 내적을 return 하도록 solution 함수를 완성해주세요.
//벡터의 곱하기는 두 가지 정의가 있는데, 내적은 벡터를 마치 수처럼 곱하는 개념이다. 벡터에는 방향이 있으므로, 방향이 일치하는 만큼만 곱한다.
...
class Solution {
    public int solution(String str1, String str2) {
        ...
        if (len1 > len2) { // str1이 str2의 부분 문자열이 될 수 없는 경우
            return 0;
        }
...
public class Solution {
    public int[] solution(int[] num_list) {
        // 배열을 ArrayList로 변환합니다.
        ArrayList<Integer> list = new ArrayList<>();
        for (int num : num_list) {
            list.add(num);
        }
        
        // 가장 작은 5개의 수를 찾아서 제거합니다.
        for (int i = 0; i < 5; i++) {
...
class Solution {
    public String[] solution(String my_string) {
        ...
        // 정렬된 접미사들을 배열에 담아 반환하기
        String[] answer = suffixes.toArray(new String[suffixes.size()]);
        return answer;
    }
}
...
class Solution {
    public String[] solution(String[] todo_list, boolean[] finished) {
        ...
        // 아직 마치지 못한 일을 찾아서 unfinished 리스트에 추가하기
        ...
        // unfinished 리스트를 배열로 변환하여 반환하기
        String[] answer = unfinished.toArray(new String[unfinished.size()]);
        return answer;
    }
}
...
class Solution {
    public int[] solution(int n) {
        ...
        // 리스트를 배열로 변환하여 반환하기
        int[] answer = new int[colList.size()];
        for (int i = 0; i < colList.size(); i++) {
            answer[i] = colList.get(i);
        }
        return answer;
    }
}
